...
@allure.story("ввод данных для доставки, нажатие на кнопку continue переход на страницу shop_totalPage.py")
class CheckoutPage:
    """
    ...
    def name_fields(self, f_name: str, l_name: str, zipp: int):
        ...
        """
        with allure.step("нахождение поля ввода имени first name {f_name}"):
            self.driver.find_element(
                By.ID, "first-name").send_keys(f_name)
        with allure.step("нахождение поля ввода фамилии last name {l_name}"):
            self.driver.find_element(
                By.ID, "last-name").send_keys(l_name)
        with allure.step("нахождение поля ввода почтовый индекс postal code {zipp}"):
            self.driver.find_element(
    ...
    @allure.description("эта функция находит кнопку и нажимает на кнопку CONTINUE")
    def cont(self):
        """
           функция нажатия на кнопку Continue
        """
        with allure.step("нахождение кнопки по CSS_SELECTOR и нажатие на нее"):
            self.driver.find_element(
